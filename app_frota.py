...
def calcular_revisao(v_info, km_atual):
    ...
    except: return "⚪ SEM DADOS", "Configure na Gestão"
...
with tabs[1 + idx_tab]:
    ...
    v_s = st.selectbox("Veículo", [""] + [f"{r['Veículo']} ({r['Placa']})" for _, r in df_v_at.iterrows()], key=f"vs_{st.session_state.reset_key}")
    ...
    if v_s and m_s:
        st_v = get_status_veiculo(v_s); v_i = df_v_at[df_v_at['Placa'] == v_s.split('(')[1].replace(')','')].iloc[0]
        s_rev, m_rev = calcular_revisao(v_i, st_v['km'])
        st.write(f"**Revisão:** {s_rev} | {m_rev}")
        if st_v["acao"] == "SAÍDA": st.error("Em uso.")
        else:
            ...
            if st.button("Confirmar Saída"):
                nova = pd.DataFrame([{"Data": get_data_hora_br(), "Ação": "SAÍDA", "Veículo": v_s, "Usuário": m_s, "KM": km_s, "Av_Saida": ", ".join(check), "Av_Totais": ", ".join(check), "Foto_Base64": converter_multiplas_fotos(fotos)}])
...
with tabs[2 + idx_tab]:
    ...
    if v_r:
        ...
        if st.button("Confirmar Chegada"):
            nova = pd.DataFrame([{"Data": get_data_hora_br(), "Ação": "CHEGADA", "Veículo": v_r, "Usuário": st_r['user'], "KM": km_f, "Av_Totais": st_r['av']}])
            salvar(pd.concat([carregar(ARQ_HIST), nova]), ARQ_HIST); st.session_state.reset_key += 1; st.rerun()
