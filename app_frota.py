    ...
    with tabs[0]:
        ...
        with c1:
            ...
            with st.form("f_veic"):
                ...
                v_rev_km = st.number_input("KM Últ. Revisão", min_value=0)
                v_rev_dt = st.date_input("Data Últ. Revisão", value=None)
                v_crit = st.selectbox("Próxima Revisão por:", ["Quilometragem", "Data (Prazo)"])
                v_val_crit = st.text_input("Valor do Critério (ex: 10000 ou 31/12/2026)")
                if st.form_submit_button("Salvar Veículo"):
                    if v_pla in df_v['Placa'].values: st.error("Placa já cadastrada!")
                    elif v_mod and v_pla:
                        nova_v = {"Veículo": v_mod, "Placa": v_pla, "Ult_Revisao_KM": v_rev_km, "Ult_Revisao_Data": v_rev_dt, "Criterio_Revisao": v_crit, "Valor_Criterio": v_val_crit, "Status": "Ativo"}
                        salvar(pd.concat([df_v, pd.DataFrame([nova_v])], ignore_index=True), ARQ_VEIC); st.rerun()
            for i, r in df_v.iterrows():
                with st.container(border=True):
                    st.write(f"**{r['Veículo']}** ({r['Placa']})")
                    b1, b2 = st.columns(2)
                    if b1.button("🚫 Bloquear/Ativar", key=f"bv{i}"):
                        df_v.at[i, 'Status'] = "Inativo" if r['Status'] == "Ativo" else "Ativo"
                        salvar(df_v, ARQ_VEIC); st.rerun()
                    if b2.button("🗑️ Excluir", key=f"dv{i}"):
        ...
        with c2:
            ...
            with st.form("f_user"):
                ...
                u_cnh = st.date_input("Validade CNH", value=None)
                u_adm = st.selectbox("Administrador?", ["Não", "Sim"])
                if st.form_submit_button("Cadastrar Usuário"):
                    if u_nome in df_m['Nome'].values: st.error("Nome já cadastrado!")
                    elif u_nome and u_cnh:
                        nova_u = {"Nome": u_nome, "Validade_CNH": u_cnh, "Status": "Ativo", "Senha": "", "Admin": u_adm}
                        salvar(pd.concat([df_m, pd.DataFrame([nova_u])], ignore_index=True), ARQ_MOT); st.rerun()
                    else: st.warning("Preencha Nome e CNH.")
            for i, r in df_m.iterrows():
                with st.container(border=True):
                    st.write(f"**{r['Nome']}** (CNH: {r['Validade_CNH']})")
                    b1, b2, b3 = st.columns(3)
                    if b1.button("🔑", key=f"rs{i}", help="Reset Senha"):
                    ...
                    if b3.button("🗑️", key=f"dm{i}", help="Excluir"):
...
with tabs[4 + idx_tab]:
    ...
    if not df_h.empty:
        ...
        with col_h1:
            idx = st.selectbox("Selecionar Linha para Ações:", df_h.index)
            if st.session_state.perfil == "admin":
