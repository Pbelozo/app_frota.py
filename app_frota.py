    ...
    with tabs[0]:
        ...
        with c1:
            ...
            with st.form("f_veic"):
                ...
                v_crit = st.selectbox("Próxima Revisão por:", ["Quilometragem", "Data (Prazo)"], index=0 if idx_v is None or str(df_v.get('Criterio_Revisao', pd.Series(['Quilometragem'])).iloc[idx_v]) == "Quilometragem" else 1)
                v_val_crit = st.text_input("Valor do Critério", value=str(df_v.get('Valor_Criterio', pd.Series([''])).iloc[idx_v]) if idx_v is not None else "")
                if st.form_submit_button("Salvar Veículo"):
                    if not v_mod or not v_pla: st.error("Preencha Modelo e Placa.")
                    else:
            ...
            for i, r in df_v.iterrows():
                with st.container(border=True):
                    ...
                    if b2.button("🗑️", key=f"dv{i}"): salvar(df_v.drop(i), ARQ_VEIC); st.rerun()
        ...
        with c2:
            ...
            with st.form("f_user"):
                u_nome = st.text_input("Nome", value=str(df_u.iloc[idx_u]['Nome']) if idx_u is not None else "")
                u_adm = st.selectbox("Admin?", ["Não", "Sim"], index=0 if idx_u is None or str(df_u.iloc[idx_u]['Admin']) == "Não" else 1)
                if st.form_submit_button("Salvar Usuário"):
                    if idx_u is not None: df_u.at[idx_u, 'Nome'] = u_nome; df_u.at[idx_u, 'Admin'] = u_adm
                    else: df_u = pd.concat([df_u, pd.DataFrame([{"Nome": u_nome, "Admin": u_adm, "Status": "Ativo", "Senha": ""}])], ignore_index=True)
            ...
            for i, r in df_u.iterrows():
                with st.container(border=True):
                    ...
                    if b2.button("🗑️", key=f"du{i}"): salvar(df_u.drop(i), ARQ_MOT); st.rerun()
        ...
        with c3:
            ...
            if st.button("Adicionar"):
                salvar(pd.concat([df_a, pd.DataFrame([{"Item": n_a, "Status": "Ativo"}])], ignore_index=True), ARQ_PECAS); st.rerun()
            st.dataframe(df_a)
...
with tabs[1 + idx_tab]:
    ...
    with col_sel:
        v_s = st.selectbox("Veículo", [""] + [f"{r['Veículo']} ({r['Placa']})" for _, r in df_v_at.iterrows()])
    ...
    if v_s and m_s:
        ...
        # Proteção contra erro de coluna ausente (KeyError)
        v_info = df_v_at[df_v_at['Placa'] == v_s.split('(')[1].replace(')','')].iloc[0]
        if 'Criterio_Revisao' in v_info and str(v_info['Criterio_Revisao']) != "":
            rev_vencida = False
            if v_info['Criterio_Revisao'] == "Quilometragem":
                try: 
                    if st_v['km'] >= int(v_info['Valor_Criterio']): rev_vencida = True
                except: pass
            else:
                try:
                    dt_rev = datetime.strptime(str(v_info['Valor_Criterio']), '%d/%m/%Y').date()
                    if date.today() >= dt_rev: rev_vencida = True
                except: pass
            if rev_vencida: st.warning("⚠️ REVISÃO VENCIDA")

        if st_v["acao"] == "SAÍDA": st.error("Veículo em uso.")
        else:
            ...
            if st.button("Confirmar Saída"):
                nova = pd.DataFrame([{"Data": get_data_hora_br(), "Ação": "SAÍDA", "Veículo": v_s, "Usuário": m_s, "KM": km_sai, "Av_Saida": ", ".join(checklist), "Av_Totais": ", ".join(checklist)}])
...
with tabs[2 + idx_tab]:
    ...
    if v_ret:
        ...
        if st.button("Confirmar Chegada"):
            nova = pd.DataFrame([{"Data": get_data_hora_br(), "Ação": "CHEGADA", "Veículo": v_ret, "Usuário": st_ret['user'], "KM": km_f, "Av_Totais": st_ret['av']}])
            salvar(pd.concat([carregar(ARQ_HIST), nova]), ARQ_HIST); st.rerun()

# --- ABA MANUTENÇÃO ---
with tabs[3 + idx_tab]:
    st.header("🔧 Oficina")
    v_m = st.selectbox("Veículo oficina", [""] + [f"{r['Veículo']} ({r['Placa']})" for _, r in carregar(ARQ_VEIC).iterrows() if get_status_veiculo(f"{r['Veículo']} ({r['Placa']})")["av"] != "Nenhuma"])
    if v_m:
