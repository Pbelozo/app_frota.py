...
def inicializar():
    ...
    if not os.path.exists(ARQ_MOT):
        ...
        if "Senha" not in dfm.columns:
            dfm["Senha"] = "" # Inicializa senhas vazias para cadastros antigos
...
def login():
    ...
    if tipo_acesso == "Administrador":
        ...
        if st.button("Entrar como Admin"):
            if senha_adm == "admin123": # Senha padrão Admin
        ...
        if nome_m:
            ...
            if dados_m['Senha'] == "":
                st.info("Primeiro acesso? Cadastre uma senha abaixo:")
                nova_senha = st.text_input("Criar Senha", type="password")
                if st.button("Cadastrar Senha e Entrar"):
...
# --- LÓGICA DAS ABAS ---
# Se for admin, a aba 0 é Gestão. Se for motorista, a aba 0 é Saída.
offset = 0 if st.session_state.perfil == "admin" else -1

# --- ABA: GESTÃO & CADASTRO (Apenas Admin) ---
if st.session_state.perfil == "admin":
    with tabs[0]:
            ...
            for i, r in df_m.iterrows():
                with st.container(border=True):
                    ...
                    if st.button("Reset Senha", key=f"rs{i}"):
                        df_m.at[i, 'Senha'] = ""
                        salvar(df_m, ARQ_MOT); st.success("Senha resetada")
                    if st.button("Bloquear", key=f"bm{i}"):
                        df_m.at[i, 'Status'] = "Inativo" if r['Status'] == "Ativo" else "Ativo"
                        salvar(df_m, ARQ_MOT); st.rerun()

        with c3:
            st.write("### 📋 Itens Checklist")
...
with tabs[1 + offset]:
    ...
    # Motorista já vem travado pelo login se for perfil motorista
    m_s = st.session_state.user_logado if st.session_state.perfil == "motorista" else st.selectbox("Selecione o Motorista", carregar(ARQ_MOT)['Nome'].tolist())
    ...
    if v_s != "Selecione...":
        ...
        if st_v["acao"] == "SAÍDA": st.error(f"Veículo com {st_v['user']}")
            ...
            checklist = st.multiselect("Confirmar Avarias Existentes:", list(set(p_lista + d_av)), default=d_av)
...
with tabs[3 + offset]:
    st.header("🔧 Oficina")
    v_m = st.selectbox("Veículo oficina", ["Selecione..."] + [f"{r['Veículo']} ({r['Placa']})" for _, r in carregar(ARQ_VEIC).iterrows()], key="of_sel")
    if v_m != "Selecione...":
...
with tabs[4 + offset]:
    ...
    df_h = carregar(ARQ_HIST)
    if not df_h.empty:
        idx_h = st.selectbox("Selecione ID para fotos:", df_h.index)
        st.dataframe(df_h.drop(columns=["Foto_Base64"]), use_container_width=True)
        fb64 = df_h.iloc[idx_h]["Foto_Base64"]
        if fb64:
