...
ARQ_LOGO = "logo_empresa.txt"

# 3. Funções de Sistema e Correção de Colunas
def inicializar():
    # Histórico
    ...
    if not os.path.exists(ARQ_VEIC):
        ...
        df = pd.read_csv(ARQ_VEIC)
        if "Status" not in df.columns:
            df["Status"] = "Ativo"
            df.to_csv(ARQ_VEIC, index=False)
    ...
    if not os.path.exists(ARQ_PECAS):
        ...
        df = pd.read_csv(ARQ_PECAS)
        if "Status" not in df.columns:
            df["Status"] = "Ativo"
            df.to_csv(ARQ_PECAS, index=False)
...
if os.path.exists(ARQ_LOGO):
    with open(ARQ_LOGO, "r") as f:
        logo_data = f.read()
    if logo_data:
        col_l1.image(base64.b64decode(logo_data), width=180)
...
with tabs[0]:
    with st.expander("🖼️ Configurar Logotipo da Empresa"):
        logo_up = st.file_uploader("Upload do Logo", type=['png', 'jpg', 'jpeg'])
        if st.button("Salvar Logotipo"):
            if logo_up:
                ...
                st.success("Logo salvo!"); st.rerun()
    ...
    with c1:
        ...
        with st.expander("➕ Novo/Editar", expanded=(v_idx != -1)):
            with st.form("f_v"):
                ...
                if st.form_submit_button("Salvar"):
                    if v_mod and v_pla and v_dt_r:
                        nova_v = {"Veículo": v_mod, "Placa": v_pla, "Ult_Revisao_KM": v_km_r, "Ult_Revisao_Data": v_dt_r, "Intervalo_KM": 10000, "Status": "Ativo"}
                        if v_idx == -1:
                            df_v = pd.concat([df_v, pd.DataFrame([nova_v])], ignore_index=True)
                        else:
                            for chave, valor in nova_v.items(): df_v.at[v_idx, chave] = valor
    ...
    with c2:
        ...
        with st.expander("➕ Novo/Editar", expanded=(m_idx != -1)):
            with st.form("f_m"):
    ...
    with c3:
        ...
        with st.expander("➕ Novo/Editar", expanded=(p_idx != -1)):
            with st.form("f_p"):
                p_desc_val = df_p.iloc[p_idx]['Item'] if p_idx != -1 else ""
                n_p_desc = st.text_input("Descrição", value=p_desc_val)
                if st.form_submit_button("Salvar Item"):
                    if n_p_desc:
                        if p_idx == -1:
                            df_p = pd.concat([df_p, pd.DataFrame([{"Item": n_p_desc, "Status": "Ativo"}])], ignore_index=True)
                        else:
...
with tabs[1]:
    ...
    v_s = st.selectbox("Veículo", ["Selecione..."] + [f"{r['Veículo']} ({r['Placa']})" for _, r in df_v_ativos.iterrows()])
    m_s = st.selectbox("Motorista", ["Selecione..."] + df_m_ativos['Nome'].tolist())
...
with tabs[3]:
    ...
    if v_m != "Selecione...":
        ...
        if st.button("🛠️ Salvar Reparo"):
            ...
            salvar(pd.concat([carregar(ARQ_HIST), nova]), ARQ_HIST); st.success("Reparo salvo!"); st.rerun()
...
with tabs[4]:
    ...
    df_h = carregar(ARQ_HIST)
    if not df_h.empty:
        idx_hist = st.selectbox("ID Linha:", df_h.index)
        st.dataframe(df_h.drop(columns=["Foto_Base64"]), use_container_width=True)
        fb64 = df_h.iloc[idx_hist]["Foto_Base64"]
        if fb64:
